...
angular.module('artoo').service('ItemsSrv', function () {
    ...
    var items = [{
       ...
       image: 'http://khazalidgrungron.altervista.org/axes/05.png',price: 3.5,
       availability: 3,
       races: ['human', 'dwarf'],
       }, 
       {
     ...
     this.remove = function (item){
         items = items.filter(singleItem => singleItem.code != item.code); // filtra tutta la lista con codice diverso dall item specificato
         
     };
     
    // this.remove = function (item){
    //     items = items.filter( function(singleItem){
    //         return singleItem.code !== item.code;       // uguale alla prima versione ma solo più grezzo
    //     }); 
    // };
